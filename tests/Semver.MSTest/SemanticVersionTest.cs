...
namespace Acklann.Semver.Tests
{
	...
	public class SemanticVersionTest
	{
		...
		public void Can_return_custom_format()
		{
			...
			v2.ToString($"x.y.z-p+YYMMHHmmss").ShouldMatch($@"{x}\.{y}\.{z}-{p}\+{now.ToString("yyMMHHmmss")}");

			string.Format("{0}", v2).ShouldBe(v2.ToString());
		}

		[TestMethod]
		public void Can_convert_to_system_version()
		{
			var semver = new SemanticVersion(1, 2, 3);
			var version = new Version(1, 2, 3);

			((Version)semver).ShouldBe(version);
			((SemanticVersion)version).ShouldBe(semver);
		}
